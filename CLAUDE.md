...
- **독립적인 작업은 병렬로 수행**: 의존성이 없는 작업들은 동시에 실행
  - 여러 파일 읽기 (Read 병렬 호출)
  - 독립적인 검색 작업 (Grep, Glob 병렬 호출)
  - 관련 없는 Bash 명령어 동시 실행
- **장시간 작업은 백그라운드 실행**: `npm install`, 빌드, 테스트 등은 `run_in_background` 사용
- **의존성 있는 작업은 순차 실행**: 이전 결과가 필요한 경우 순서대로 진행

### 작업 완료 검증

- 코드 수정 후 반드시 동작 확인
- 에러 발생 시 원인 분석 후 수정
- 변경사항 적용을 위해 서버 재시작 필요 여부 확인
